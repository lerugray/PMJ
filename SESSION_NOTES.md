...
3. **Cursor-based map highlighting** — when scrolling through the move destination list or contact target list, the selected location lights up on the map (white on blue). Works for move destinations, contact location select, and contact target select.
